// exportando uma função
export default function ehUmCPF(campo) {
  // pegando o valor inserido no cpf e substituindo . ou - por vazio
  const cpf  = campo.value.replace(/\.|-/g, "")

  // verificando se o cpf não existe
  if (validaNumerosRepetidos(cpf) || validaPrimeiroDigito(cpf) || validaSegundoDigito(cpf)) {
    // Com o método setCustomValidity é possível alterar o valor de customError
    campo.setCustomValidity("Esse cpf não é valido")
  }
}

// verifica se o cpf não esta com todos os numeros repetidos
function validaNumerosRepetidos(cpf) {
  const numeroRepetidos = [
    '00000000000',
    '11111111111',
    '22222222222',
    '33333333333',
    '44444444444',
    '55555555555',
    '66666666666',
    '77777777777',
    '88888888888',
    '99999999999'
  ]

  // se o cpf estiver entre os numeros repetidos, retorna true
  return numeroRepetidos.includes(cpf)
}

function validaPrimeiroDigito(cpf) {
  let soma = 0
  let multiplicador = 10

  for(let tamanho = 0; tamanho < 9; tamanho++) {
    soma += cpf[tamanho] * multiplicador;
    multiplicador --;
  }

  soma = (soma * 10) % 11

  if (soma == 10 || soma == 11) {
    soma = 0
  }

  return soma != cpf[9];
}

function validaSegundoDigito(cpf) {
  let soma = 0
  let multiplicador = 11

  for(let tamanho = 0; tamanho < 10; tamanho++) {
    soma += cpf[tamanho] * multiplicador;
    multiplicador --;
  }

  soma = (soma * 10) % 11

  if (soma == 10 || soma == 11) {
    soma = 0
  }

  return soma != cpf[10];
}